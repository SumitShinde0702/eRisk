...
The task requires a doctor agent to conduct an indirect, natural-language interview with 20 simulated patient personas (Llama-3-8B with per-persona LoRA adapters) and to predict each persona's Beck Depression Inventory--II (BDI-II) total score together with up to four key symptoms.
...
Post-hoc evaluation against the released ground truth (19 personas submitted) yields ADODL scores of 0.81--0.85; Run~1 achieves the highest average symptom hit rate (ASHR = 0.276) among incomplete submissions in the preliminary results report.
...
eRisk 2026 Task~1 extends this pilot into a full shared task with 20 patient personas, weekly LoRA-adapter releases on top of \texttt{Meta-Llama-3-8B-Instruct}~\cite{Dubey2024llama3}, and an explicit evaluation framework targeting BDI-II total scores and symptom identification~\cite{PerezEtAl2026eRiskWorkingNotes}.
...
Our official submissions cover personas 1--19 ($n=19$); persona~20 was not submitted.
...
After the submission deadline, reference BDI-II scores and key symptoms for all 20 personas were released for post-hoc analysis.\footnote{Ground-truth labels: \url{https://drive.google.com/drive/folders/1FxKelhBT9uXaGLH_-3fRkTkVv8KMAGXo?usp=sharing}. Each row in \texttt{patients\_data.jsonl} uses \texttt{patient\_id}~$1,\ldots,20$, matching the persona index in our submission JSON (\texttt{LLM} field) and FTP folder names.}
...
The organisers released reference BDI-II totals and up to four key symptoms per persona after the submission window closed (May 2026) and published preliminary Task~1 results for team \textbf{INSALyon-2} (19 personas per run).
We reproduce their metrics on our submitted \texttt{outputs/submission/} files by matching each persona~$p \in \{1,\ldots,19\}$ directly to ground-truth row \texttt{patient\_id}~$=~p$ in \texttt{patients\_data.jsonl}, and normalising predicted symptom strings with the official \texttt{symptom\_mappings.json} synonym map.

Following the preliminary report, we compute:
\textbf{ADODL} (Average Difference between Overall Depression Levels), $\mathrm{ADODL} = \frac{1}{|U|}\sum_u (63 - |\mathrm{ADL}_u - \mathrm{EDL}_u|)/63$;
\textbf{DCHR} (Depression Category Hit Rate), the fraction of personas whose predicted BDI-II total falls in the same severity band as the reference (minimal 0--9, mild 10--18, moderate 19--29, severe 30--63);
and \textbf{ASHR} (Average Symptom Hit Rate), the mean fraction of the four reference key symptoms correctly identified after normalisation.

Table~\ref{tab:gt} shows that our reproduced scores match the organisers' preliminary report exactly for ADODL and DCHR; ASHR differs by at most 0.013 on runs 2--3 due to minor symptom-string normalisation details.
Run~1 attains the \emph{highest ASHR among all 19-persona submissions} in the preliminary incomplete-submission table (0.2763).
Run~3 achieves our best ADODL (0.8496).

\begin{table}[t]
\caption{Official Task~1 metrics reproduced on INSALyon-2 submissions ($n = 19$ personas; persona~20 not submitted). Organiser preliminary values in parentheses where available.}
\label{tab:gt}
\centering
\small
\begin{tabular}{lccc}
\toprule
\textbf{Run} & \textbf{ADODL} & \textbf{DCHR} & \textbf{ASHR} \\
\midrule
...
Although raw BDI-II MAE remains moderate (9.5--11.7 points on this subset), ADODL is high because errors are normalised against the 0--63 scale: a 10-point miss still yields $(63-10)/63 \approx 0.84$.
Our system therefore tracks overall depression level well, with Run~1 particularly strong on symptom identification (ASHR), consistent with the organisers' feedback that our results are accurate relative to other partial submissions.
Latency-aware scores (LDCHR/LASHR) are lower in the preliminary report because our runs average 23.4 doctor messages per conversation---a deliberate trade-off for structured group screening and bank follow-ups.
...
Post-hoc evaluation against the released ground-truth labels (Section~\ref{sec:gt-eval}) complements our TalkDep development analysis; our reproduced ADODL scores (0.81--0.85 on 19 personas) align with the organisers' preliminary INSALyon-2 report.
