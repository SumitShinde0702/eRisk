...
The task requires a doctor agent to conduct an indirect, natural-language interview with 19 simulated patient personas (Llama-3-8B with per-persona LoRA adapters) and to predict each persona's Beck Depression Inventory--II (BDI-II) total score together with up to four key symptoms.
...
Post-hoc evaluation against the organisers' released ground-truth labels (19 personas) reports MAE of 9.5--11.7 and symptom F1 of 0.22--0.28 across our three runs.
...
eRisk 2026 Task~1 extends this pilot into a full shared task with 19 patient personas, weekly LoRA-adapter releases on top of \texttt{Meta-Llama-3-8B-Instruct}~\cite{Dubey2024llama3}, and an explicit evaluation framework targeting BDI-II total scores and symptom identification~\cite{PerezEtAl2026eRiskWorkingNotes}.
...
eRisk 2026 Task~1 (Conversational Depression Detection with LLM Personas) requires participants to interact with 19 simulated patient personas and predict, for each persona, a BDI-II total score (0--63) and up to four key depressive symptoms.
...
After the submission deadline, reference BDI-II scores and key symptoms for all 19 task personas were released for post-hoc analysis.\footnote{Ground-truth labels: \url{https://drive.google.com/drive/folders/1FxKelhBT9uXaGLH_-3fRkTkVv8KMAGXo?usp=sharing}}
...
We evaluate our three official runs post hoc by comparing each \texttt{results\_run\{N\}.json} file against \texttt{patients\_data.jsonl} for personas 1--19, normalising predicted symptom strings with the official \texttt{symptom\_mappings.json} synonym map.
...
Table~\ref{tab:gt} summarises results on the full set of 19 task personas.

\begin{table}[t]
\caption{Post-hoc evaluation against released eRisk 2026 ground truth ($n = 19$ personas).}
\label{tab:gt}
\centering
\small
\begin{tabular}{lcccc}
...
\midrule
Run~1 (balanced)   & 11.68 & 14.28 & 0.118 & 0.280 \\
Run~2 (recall)     &  9.68 & 12.75 & 0.338 & 0.239 \\
Run~3 (precision)  &  9.47 & 12.20 & 0.382 & 0.216 \\
\bottomrule
\end{tabular}
\end{table}

Run~3 achieves the lowest score error on this subset, while Run~1 attains the highest symptom F1.
The moderate rank correlations indicate that absolute calibration remains challenging on the official personas despite strong TalkDep extractor performance (Section~\ref{sec:talkdep}), motivating further analysis of persona-specific conversational dynamics and extractor generalisation in the camera-ready version.
...
Post-hoc evaluation against the released ground-truth labels (Section~\ref{sec:gt-eval}) complements our TalkDep development analysis on the complete set of 19 task personas.
