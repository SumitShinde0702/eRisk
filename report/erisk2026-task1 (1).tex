...
eRisk 2026 Task~1 extends this pilot into a full shared task with 20 patient personas, weekly LoRA-adapter releases on top of \texttt{Meta-Llama-3-8B-Instruct}~\cite{Dubey2024llama3}, and an explicit evaluation framework targeting BDI-II total scores and symptom identification.
...
\textbf{Evaluation.}
The organisers evaluate (i)~accurate identification of depressive symptoms present in the persona and (ii)~the overall depression level following BDI-II standards~\cite{Parapar2025erisk}.
...
Official eRisk test labels are hidden at submission time.
We report internal evaluation on three axes: TalkDep corpus calibration, component ablation, and symptom difficulty analysis.

\textbf{Important caveat.}
All numbers in this section are \emph{development proxies}; they do not substitute for the official eRisk evaluation.
...
\textbf{Label access.}
Official reference BDI-II scores for the 20 eRisk personas are hidden; reported metrics on TalkDep are proxies that may not reflect performance on the test personas.
...
We submit three runs (balanced, recall-oriented, precision-oriented) and look forward to evaluating them against the official eRisk 2026 test labels.
